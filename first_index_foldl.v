...
Local Lemma hd_skip : forall {A} (l : list A) n, hd_error (skipn n l) = nth_error l n.
Proof.
  induction l; Reconstr.scrush.
Qed.

Local Lemma first_add : forall {A} (l : list A) (n : nat), firstn (S n) l = (firstn n l) ++ (match nth_error l n with | Some e => (e :: nil) | None => nil end).
Proof. induction l.
 - Reconstr.scrush.
...
Local Lemma Forall_last : forall {A} (l : list A) (P : A -> Prop) (x : A), Forall P l /\ P x <-> Forall P (l ++ (x :: nil)).
Proof.
 induction l; Reconstr.scrush.
Qed.

Local Lemma skip_one : forall {A} l tail (head : A) n, head :: tail = skipn n l -> tail = skipn (S n) l.
Proof.
 induction l.
 - induction tail.
   + auto.
